import LEETCODE_SVG_ICON from "../assets/leetcode.svg"

const PopoverTriggerData = ({eventInfo}) => {
  return (<>
    {eventInfo.event.extendedProps?.platform === "geeksforgeeks" && <svg className='m-0.5 px-1 w-26 bg-black rounded-full my-auto' xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="45" height="30" viewBox="0 0 48 48">
        ...
        { eventInfo.event.extendedProps?.platform === "codechef" &&  <img className='w-8 my-auto ' src="https://cdn.codechef.com/images/cc-logo-mobile-1.svg" alt="" /> }
        {
                eventInfo.event.extendedProps?.platform === "leetcode" && <img className='max-h-18 m-0.5 p-1 bg-gray-500 rounded-full' width="28" height="36" src={LEETCODE_SVG_ICON} alt="external-level-up-your-coding-skills-and-quickly-land-a-job-logo-shadow-tal-revivo"/>  }
        {
               eventInfo.event.extendedProps?.platform === "atcoder" && <img className='my-auto bg-black p-0.5 rounded-full m-1.5' width="30" height="46" src="https://codolio.com/icons/atcoder_dark.png"/>  }
                 {eventInfo.event.extendedProps.platform === "codeforces"&& <img className="m-0.5 w-7 p-1   bg-black rounded-full" src="https://codolio.com/icons/codeforces.png" width={25} alt = "Platform Logo" />}
                  <span className="overflow-hidden font-semibold text-black">{eventInfo.event.extendedProps.contestName} </span>
        </>
  )
}

export default PopoverTriggerData
